...
// All recomputes (score/gaps/exposures/carriers) are live via useMemo on profile+policy change.
...
export default function PolicyLens() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      setIsDlqEnabled(new URLSearchParams(window.location.search).get("dlq") === "1"); // eslint-disable-line react-hooks/set-state-in-effect
    }
  }, []);

  // Derived: only real policies (isInsuranceDocument true) produce dashboard.
  const currentPolicy: PolicyData | null = useMemo(() => {
    if (extracted && "isInsuranceDocument" in extracted && extracted.isInsuranceDocument) {
  ...
  const scoreResult: ScoreResult = useMemo(() => {
    if (!currentPolicy) {
      return { coverageScore: 0, gaps: [], exposures: [], coveragesPresentCount: 0 };
    }
  ...
  const savingsInfo = useMemo(() => {
    const currentPrem = currentPolicy?.annualPremiumTotal ?? null;
    return computeSavings(carrierMatches, currentPrem);
  }, [carrierMatches, currentPolicy]);

  // UI conditionals (drive the 4 top-level render branches: hero, not-ins, loading, dashboard).
  const hasData = currentPolicy !== null;
  const isNotInsurance = extracted && "isInsuranceDocument" in extracted && !extracted.isInsuranceDocument;
  const isFallback = isNotInsurance && !!(extracted as Partial<FallbackDoc>)._fallback;
  ...
  // Also resets profile so sample always shows known 74/2gaps/16% numbers.
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-[1100px] mx-auto px-6 py-5 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-2xl font-semibold tracking-[-0.5px] text-[#16223b]">PolicyLens</span>
            </div>
            <div className="text-xs text-[#64748b] mt-0.5">Coverage intelligence for commercial insurance.</div>
          </div>

          <div className="flex items-center gap-4 text-sm">
            <span className="text-[#64748b] hidden sm:inline">Transparent • Deterministic • No data leaves for demo</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-[1100px] mx-auto px-6 pt-8 pb-16">
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
          ...
          <div className="flex items-center gap-2">
            <UploadDropzone onFile={handleUpload} disabled={isLoading} onError={setUploadError} />
            <button
              onClick={loadSample}
            ...
            {hasData && (
              <button onClick={clearData} className="btn btn-secondary text-xs px-3">Clear</button>
            )}
        ...
        {!hasData && !isLoading && !isNotInsurance && (
          <div className="card p-12 text-center mt-2">
            <div className="mx-auto w-12 h-12 rounded-full bg-[#f7f9fc] flex items-center justify-center mb-4">📋</div>
            <h2 className="text-xl font-semibold tracking-tight mb-2">Upload a policy or load the sample</h2>
            <p className="max-w-md mx-auto text-[#64748b] mb-6">
              PolicyLens extracts coverages from a commercial COI or declarations page, scores risk posture, flags gaps, and ranks carriers against your profile.
            </p>
            <div className="flex justify-center gap-3">
              <button onClick={loadSample} className="btn btn-primary">Load sample policy</button>
              <label className="btn btn-secondary cursor-pointer">
                Upload PDF
                <input type="file" accept=".pdf" className="hidden" onChange={(_e) => {
                  const f = (_e.target as HTMLInputElement).files?.[0];
                  if (f) handleUpload(f);
                }} />
              </label>
            </div>
            <div className="text-[11px] text-[#64748b] mt-6">Sample data is bundled. Real uploads use server-side LLM extraction (key stays private).</div>
          </div>
        )}
        ...
        {isNotInsurance && (
          <div className="card border-[#f59e0b] bg-[#fefce8] p-6 mb-6">
            <div className="font-semibold mb-1">We couldn&apos;t find policy data in that PDF.</div>
            <div className="text-sm text-[#475569]">
              Try a Certificate of Insurance or a declarations page, or load the sample.
              {isFallback && " (LLM not configured — demo using sample path.)"}
            </div>
            <button onClick={loadSample} className="mt-3 btn btn-primary text-sm">Load sample policy</button>
          </div>
        )}
        ...
        {hasData && !isLoading && (
          <>
            ...
            <StatCards
              ...
              savingsPct={savingsInfo.avgSavingsPct}
              savingsMessage={savingsInfo.message}
            />
            ...
            {/* Carrier Match */}
            <CarrierTable matches={carrierMatches} savingsMessage={savingsInfo.message} />

            {/* Footer note */}
            <div className="text-[11px] text-[#64748b] mt-8 px-1">
              Changing industry or revenue instantly recomputes the score, gaps, exposures and carrier rankings. All scoring logic is pure functions in <code className="font-mono text-xs">lib/scoring.ts</code>.
            </div>
          </>
        )}
        ...
        {isDlqEnabled && showDlq && (
          <div className="mt-8 p-4 border border-amber-200 bg-amber-50/60 rounded text-xs">
            <div className="font-semibold mb-2 text-amber-800 flex items-center justify-between">
              ...
              <button onClick={() => setShowDlq(false)} className="text-amber-600">×</button>
            </div>
            {dlqLoading && <div>Loading DLQ…</div>}
            {!dlqLoading && dlqItems.length === 0 && <div className="text-amber-700">No entries yet. Failures during uploads/LLM/validation will appear here.</div>}
            <ul className="space-y-2">
              {dlqItems.map((item, idx) => {
                const r = item as { id?: string; reason?: string; timestamp?: string; filename?: string; rawTextSnippet?: string; error?: string };
                return (
                  <li key={r.id || idx} className="bg-white/70 p-2 rounded border border-amber-100 font-mono break-all">
                    <div><strong>{r.reason}</strong> @ {r.timestamp}</div>
                    {r.filename && <div>file: {r.filename}</div>}
                    <div className="text-[10px] text-amber-700 mt-0.5">snippet: {r.rawTextSnippet?.slice(0, 160)}…</div>
                    {r.error && <div className="text-red-600">err: {r.error}</div>}
                  </li>
                );
              })}
            </ul>
            <div className="mt-2 text-[10px] text-amber-600">Full data also at <code>/api/dlq</code> and in <code>data/dlq/</code> (dev-gated by ?dlq=1; JSONL append-only)</div>
          </div>
        )}
      </main>
